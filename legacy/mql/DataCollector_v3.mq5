//+------------------------------------------------------------------+
//|  DataCollector_v3.mq5                                            |
//|  Range-based features — กำหนดช่วง period ของ indicator ได้      |
//|                                                                  |
//|  3 Modes ของแต่ละ indicator:                                    |
//|    OFF        : ไม่เก็บ                                          |
//|    SINGLE     : 1 ค่า (เหมือน v2)                                |
//|    MULTI      : เก็บหลาย periods แบบ strategic (3-5 ค่า)        |
//|    AGGREGATE  : เก็บ min/max/mean/std ของ range  ⭐ recommended  |
//|    RANGE_ALL  : เก็บทุก period ใน range  ⚠️ overfit risk         |
//|                                                                  |
//|  ตัวอย่าง:                                                       |
//|    RSI_Mode = AGGREGATE, RSI_Min=4, RSI_Max=30                  |
//|    → เก็บ rsi_min, rsi_max, rsi_mean, rsi_std (4 features)      |
//+------------------------------------------------------------------+
#property copyright "PyCaret Trainer v3 (Range Features)"
#property version   "3.00"
#property strict

enum FeatureMode {
   FEAT_OFF = 0,        // ไม่เก็บ
   FEAT_SINGLE = 1,     // 1 period เดียว
   FEAT_MULTI = 2,      // หลาย periods (strategic)
   FEAT_AGGREGATE = 3,  // min/max/mean/std ของ range
   FEAT_RANGE_ALL = 4,  // ทุก period ใน range (overfit risk!)
};

//=== Output ===
input string FileName        = "training_data_v3.csv";
...
input int    RSI_Min         = 4;       // start period
input int    RSI_Max         = 30;      // end period
input int    RSI_Step        = 1;       // step (for RANGE_ALL)
input string RSI_Multi       = "7,14,21,28";  // for MULTI mode
...
//+------------------------------------------------------------------+
//| Build periods array based on mode                                |
//+------------------------------------------------------------------+
void BuildPeriods(FeatureMode mode, int p_min, int p_max, int p_step,
                   string multi_str, int single_default,
                   int &periods[]) {
   ...
   if(mode == FEAT_SINGLE) {
      ...
      // เก็บทุก period ในช่วง (จะใช้คำนวณ aggregate ทีหลัง)
      int n = (p_max - p_min) / p_step + 1;
      ArrayResize(periods, n);
      for(int i = 0; i < n; i++) {
         periods[i] = p_min + i * p_step;
      }
   }
}

//+------------------------------------------------------------------+
//| Create indicator handles for periods                             |
//+------------------------------------------------------------------+
bool CreateRSIHandles(int &periods[], int &handles[]) {
   ...
   for(int i = 0; i < n; i++) {
      ...
      if(handles[i] == INVALID_HANDLE) {
         Print("[v3] RSI handle period=", periods[i], " failed");
         return false;
      }
   }
   return true;
...
//+------------------------------------------------------------------+
//| Calc aggregate stats from values array                            |
//+------------------------------------------------------------------+
...
void BuildHeader() {
   ...
   AddIndicatorCols("stoch", Stoch_Mode, g_stoch_periods);  // K only
   ...
   // Other static columns (no range version)
   string others[] = {
...
void AddIndicatorCols(string name, FeatureMode mode, int &periods[]) {
   ...
   if(mode == FEAT_AGGREGATE) {
      ...
      // SINGLE / MULTI / RANGE_ALL: 1 column per period
...
int OnInit()
{
   ...
   if(g_handle == INVALID_HANDLE) {
      Print("[v3] เปิดไฟล์ไม่ได้: ", GetLastError());
      return INIT_FAILED;
   }

   // Build periods arrays
   ...
   Print("[v3] periods | RSI=", ArraySize(g_rsi_periods),
   ...
   // Other indicators (always single)
   ...
   Print("[v3] Total features: ", ArraySize(g_header_cols) - 7 - 2,
...
void OnDeinit(const int reason)
{
   ...
   IndicatorRelease(h_ema_long); IndicatorRelease(h_macd); IndicatorRelease(h_bb);
   Print("[v3] เขียนแล้ว ", g_rows, " rows");
}
...
//+------------------------------------------------------------------+
//| Write feature columns based on mode                              |
//+------------------------------------------------------------------+
void WriteIndicatorValues(FeatureMode mode, int &handles[],
   ...
   for(int i = 0; i < n; i++) {
      values[i] = GetBuf(handles[i], buffer_idx, shift);
   }
   ...
   if(mode == FEAT_AGGREGATE) {
      ...
      for(int i = 0; i < n; i++) {
         output += "," + DoubleToString(values[i], 4);
      }
   }
}
...
void OnTick()
{
   ...
   int max_window = MathMax(Stat_Window + 1, Rank_Window);  // +1 for Sharpe j+1
   ...
   // Request enough bars for ALL operations (Stat_Window for ZScore/Sharpe, Rank_Window for PctRank)
   int rates_needed = i + max_window + ForwardBars + 5;

   MqlRates rates[];
   int copied = CopyRates(_Symbol, _Period, 0, rates_needed, rates);
   if(copied < rates_needed) return;  // not enough data yet — wait
   ...
   // Final safety: ensure closes array is big enough for max access
   ...
   WriteIndicatorValues(Stoch_Mode, g_h_stoch, 0, i, row);  // %K main buffer
   WriteIndicatorValues(CCI_Mode,   g_h_cci,   0, i, row);
   WriteIndicatorValues(WPR_Mode,   g_h_wpr,   0, i, row);
   WriteIndicatorValues(ADX_Mode,   g_h_adx,   0, i, row);  // main ADX buffer
   ...
   FileFlush(g_handle);
   g_rows++;
   if(g_rows % LogEvery == 0) Print("[v3] เขียนแล้ว ", g_rows, " rows");
}
//+------------------------------------------------------------------+
