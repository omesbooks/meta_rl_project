...
//|   [0] Doji         — 0 / 1                                       |
//|   [1] Hammer       — -1 (shooting star) / 0 / +1 (hammer)        |
//|   [2] Engulfing    — -1 / 0 / +1                                 |
//|   [3] InsideBar    — 0 / 1                                       |
//|   [4] OutsideBar   — 0 / 1                                       |
//|   [5] Star         — -1 (evening) / 0 / +1 (morning)             |
//|   [6] Soldiers     — -1 (3 black crows) / 0 / +1 (3 white)       |
//|   [7] Marubozu     — -1 / 0 / +1                                 |
//|   [8] Harami       — -1 / 0 / +1                                 |
//|   [9] Piercing     — -1 (dark cloud) / 0 / +1 (piercing)         |
//+------------------------------------------------------------------+
#property strict
...
#property indicator_label2  "Hammer"
#property indicator_type2   DRAW_NONE
#property indicator_label3  "Engulfing"
#property indicator_type3   DRAW_NONE
#property indicator_label4  "InsideBar"
#property indicator_type4   DRAW_NONE
#property indicator_label5  "OutsideBar"
#property indicator_type5   DRAW_NONE
#property indicator_label6  "Star"
#property indicator_type6   DRAW_NONE
#property indicator_label7  "Soldiers"
#property indicator_type7   DRAW_NONE
#property indicator_label8  "Marubozu"
#property indicator_type8   DRAW_NONE
#property indicator_label9  "Harami"
#property indicator_type9   DRAW_NONE
#property indicator_label10 "Piercing"
#property indicator_type10  DRAW_NONE

//=== Pattern toggles (enable/disable per pattern) ===
input group "=== Pattern Toggles ==="
input bool InpEnableDoji       = true;     // [0] Doji
input bool InpEnableHammer     = true;     // [1] Hammer / Shooting Star
input bool InpEnableEngulfing  = true;     // [2] Bullish / Bearish Engulfing
input bool InpEnableInsideBar  = true;     // [3] Inside Bar
input bool InpEnableOutsideBar = true;     // [4] Outside Bar
input bool InpEnableStar       = true;     // [5] Morning / Evening Star
input bool InpEnableSoldiers   = true;     // [6] Three Soldiers / Crows
input bool InpEnableMarubozu   = true;     // [7] Marubozu
input bool InpEnableHarami     = true;     // [8] Harami
input bool InpEnablePiercing   = true;     // [9] Piercing / Dark Cloud
...
input bool InpDrawDojiArrow        = false;    // Doji is too common — skip arrow
...
int OnInit()
{
   ...
   SetIndexBuffer(0, BufDoji,        INDICATOR_DATA);
   SetIndexBuffer(1, BufHammer,      INDICATOR_DATA);
   SetIndexBuffer(2, BufEngulfing,   INDICATOR_DATA);
   SetIndexBuffer(3, BufInsideBar,   INDICATOR_DATA);
   SetIndexBuffer(4, BufOutsideBar,  INDICATOR_DATA);
   SetIndexBuffer(5, BufStar,        INDICATOR_DATA);
   SetIndexBuffer(6, BufSoldiers,    INDICATOR_DATA);
   SetIndexBuffer(7, BufMarubozu,    INDICATOR_DATA);
   SetIndexBuffer(8, BufHarami,      INDICATOR_DATA);
   SetIndexBuffer(9, BufPiercing,    INDICATOR_DATA);
           ...
           double o2, double h2, double l2, double c2)
{
   ...
   if(c2 < o2 && body2 > range1 * 0.7 &&
      c0 > o0 && body0 > range1 * 0.7 && c0 > mid2)
                ...
                const int &spread[])
{
   ...
   for(int i = start; i < rates_total; i++) {
      ...
      BufDoji[i]     = InpEnableDoji     ? IsDoji(o0, h0, l0, c0)         : 0;
      ...
      if(InpDrawArrows && i < rates_total - 1) {
         if(BufStar[i] != 0)
            DrawArrow(time[i], (int)BufStar[i], "Star");
         else if(BufSoldiers[i] != 0)
            DrawArrow(time[i], (int)BufSoldiers[i], "3Soldiers");
         ...
         else if(InpDrawDojiArrow && BufDoji[i] != 0) {
            // Doji has no direction — small dot above bar
            string name = StringFormat("CP_Doji_%I64d", (long)time[i]);
            if(ObjectFind(0, name) < 0) {
               int shift = iBarShift(_Symbol, _Period, time[i]);
               double y = iHigh(_Symbol, _Period, shift) + 5 * _Point;
               ObjectCreate(0, name, OBJ_ARROW, 0, time[i], y);
               ObjectSetInteger(0, name, OBJPROP_ARROWCODE, 158);
               ObjectSetInteger(0, name, OBJPROP_COLOR, clrYellow);
               ObjectSetString(0, name, OBJPROP_TOOLTIP, "Doji");
            }
         }
      }
   }
