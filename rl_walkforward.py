...
def main():
    ...
    n = len(df)
    # Define windows
    # 60% test reserved at end starts at 50%, train always grows
    # Test slice = 10% of data each
    test_size = int(n * 0.10)
    windows = []
    for i in range(args.windows):
        train_end = int(n * (0.50 + i * 0.10))   # 50%, 60%, ..., 90%
        test_end = train_end + test_size
        if test_end > n:
            test_end = n
        if args.mode == "rolling":
            train_start = int(n * (0.0 + i * 0.10))
        else:  # anchored
            train_start = 0
    ...
    for w_idx, (ts, te, vs, ve) in enumerate(windows, 1):
        print(f"\n{'=' * 60}")
        print(f"  Window {w_idx}/{args.windows}")
        print(f"{'=' * 60}")
        win_start = time.time()
    ...
    if n_pos == args.windows and n_wr == args.windows:
        verdict = "✅ ROBUST — ทุก window ผ่าน (PF>1 และ WR>50%) → edge จริง"
    elif n_pos >= args.windows * 0.7:
        verdict = f"🟡 MOSTLY ROBUST — {n_pos}/{args.windows} windows มี PF>1 → ใช้ระวัง"
    elif n_pos >= args.windows * 0.5:
        verdict = f"⚠️ UNSTABLE — แค่ {n_pos}/{args.windows} windows ผ่าน → over-fit เป็นไปได้สูง"
    else:
        verdict = f"❌ NOT ROBUST — {n_pos}/{args.windows} windows ผ่าน → edge ไม่จริง / ขึ้นอยู่กับ luck"
    print(f"    {verdict}")
    ...
    try:
        ...
        colors = plt.cm.viridis(np.linspace(0, 0.9, args.windows))
        ...
        axes[0].set_title(f"Walk-Forward Equity Curves — {args.windows} windows")
        axes[0].set_ylabel("Equity (normalized)")
        axes[0].legend(loc="best", fontsize=9)
        axes[0].grid(True, alpha=0.3)

        # Bottom: WR + PF bar chart
        x = np.arange(args.windows) + 1
        ax2 = axes[1]
        ax2_twin = ax2.twinx()
